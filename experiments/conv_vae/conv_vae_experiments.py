from autoencoders import MnistConvolutionalVariationalAutoencoder, Cifar10ConvolutionalVariationalAutoencoder
...
if args.dataset == 'mnist':
  ...
  base_logdir = 'fashion_mnist_cvae'
...
for latent_dim in range(min_latent_dim, max_latent_dim+1):
  ...
  if args.dataset == 'mnist' or args.dataset == 'fashion_mnist':
    model = MnistConvolutionalVariationalAutoencoder(input_dims=input_dims, latent_dim=latent_dim)
  elif args.dataset == 'cifar10':
    model = Cifar10ConvolutionalVariationalAutoencoder(input_dims=input_dims, latent_dim=latent_dim)
  ...
  model.fit(train_dataset,
            ...
            callbacks=[SaveLossesCallback(logdir=base_logdir+'/logs/cvae_{}/'.format(str(latent_dim))),
                      ReconstructionErrorCallback(logdir=base_logdir+'/logs/cvae_{}/'.format(str(latent_dim)))]
            )
  end_time = time.time()
  model.save_weights(base_logdir+'/weights/cvae_{}.ckpt'.format(str(latent_dim)))
